...
public class CustomerServiceLocationsPage {
    ...
    private final SelenideElement nextPageButton = $x("//*[name()='svg'][@data-testid='icon_baseX24/arrowRight']/parent::*");
    private final SelenideElement nextPageMobileButton = $x("//div/nav/button[last()]");
    ...
    public void processingCardsOfTerminalsAndATMs() {
        SelenideElement nextPageButtonVariable;
        if (Objects.equals(Configuration.browserSize, "430x932")) {
            nextPageButtonVariable = nextPageMobileButton;
        } else {
            nextPageButtonVariable = nextPageButton;
        }
        while (nextPageButtonVariable.isEnabled()) {
            ...
            nextPageButtonVariable.click();
            if (!nextPageButtonVariable.isEnabled()) {
                processingCardsOfTerminalsAndATMs();
            }
        }
